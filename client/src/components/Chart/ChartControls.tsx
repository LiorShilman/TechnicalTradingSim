import { ChevronLeft, RotateCcw, Play, Pause, Save, History, HelpCircle, Bot } from 'lucide-react'
...
export default function ChartControls() {
  ...
  return (
    <div className="flex flex-col lg:flex-row items-start lg:items-center lg:justify-between w-full gap-2 lg:gap-0">
      {/* ימין (desktop) / שני (mobile): מידע + עזרה/היסטוריה */}
      <div className="flex flex-wrap items-center gap-2 lg:gap-3 w-full lg:w-auto order-2 lg:order-1">
        ...
        <button
          ...
          disabled={!gameState || isLoading}
          className={`px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition-all shadow-md hover:shadow-lg ${
            isDemoMode
              ? 'bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 ring-2 ring-green-400 ring-offset-2 ring-offset-dark-bg'
              : 'bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700'
          } disabled:bg-dark-border disabled:cursor-not-allowed`}
          title={isDemoMode ? 'עצור AI Demo' : 'הפעל AI Demo - המחשב ישחק'}
        >
          <Bot size={20} className={isDemoMode ? 'animate-pulse' : ''} />
          AI Demo
        </button>
      </div>

      {/* שמאל (desktop) / ראשון (mobile): כפתורי בקרה */}
